...
import { FlowResult, FlowState, FLOW_STATES } from '../src/modules/ia/conversation/flow.types';
import { NucleusInput, Nucleus } from '../src/modules/ia/nuclei/nucleus.interface';
...
class InMemorySessionService {
  ...
  async updateSessionVersioned(userId: string, data: { state?: string; payload?: any; expiresAt?: Date }, expectedVersion: number) {
    const s = this.store.get(userId);
    if (!s) throw new Error('session not found');
  ...
  async touchSession(userId: string, extraMin?: number) { return null; }
  async deleteSession(userId: string) { this.store.delete(userId); }
  async cleanupExpired() { return null; }
}
  ...
  getGoalsForUser: async () => [
    {
      ...
      plantedTree: {
        ...
        growthEvents: [
          { id: 'e1', stage: 1, createdAt: new Date().toISOString(), title: 'Planted', description: '', progressIndex: 1 },
        ],
      },
    },
  ],
...
jest.mock('../src/prisma/client', () => ({
  prisma: {
    user: { findUnique: jest.fn(async ({ where }) => ({ id: where.id, currentWorldId: 'mundo1' })) },
  },
}));

// helper to build orchestrator with custom nuclei/router
function makeOrchestrator(nuclei: Partial<Record<FlowState, Nucleus>>, router: { analyze(input: NucleusInput): Promise<RouterResult> }) {
  const clar = nuclei[FLOW_STATES.CLARIFICATION] || ({ analyze: async () => ({ actions: [], decision: 'handled', confidence: 1 }) } as Nucleus);
  ...
  const nucleusMetaFactory = new NucleusMetaFactory(goalStatusMetaBuilder);
...
describe('ConversationOrchestrator contract tests', () => {
  it('throws when nucleus returns invalid decision', async () => {
    const badNuc: Nucleus = { analyze: async () => ({ actions: [], decision: 'foo' as any, confidence: 1 }) };
    const orch = makeOrchestrator({ [FLOW_STATES.CLARIFICATION]: badNuc }, { analyze: async () => ({ target: null, confidence: 1 }) });
    await expect(orch.handle({ userId: 'u1', userMessage: 'x' })).resolves.toMatchObject({
  ...
  it('router invalid confidence degrades gracefully to clarification', async () => {
    ...
    const nuc: Nucleus = { analyze: async () => ({ actions: [], decision: 'not_my_job', confidence: 0.5 }) };
    const badRouter = { analyze: async () => ({ target: FLOW_STATES.GOAL_STATUS as any, confidence: 2 }) };
    const clar: Nucleus = { analyze: async () => ({ actions: [{ type: 'reply', text: 'clar' }], decision: 'handled', confidence: 1 }) };
    const orch = makeOrchestrator({ [FLOW_STATES.GOAL_CREATION]: nuc, [FLOW_STATES.CLARIFICATION]: clar }, badRouter);
  ...
  it('not_my_job with actions throws', async () => {
    const nuc: Nucleus = { analyze: async () => ({ actions: [{ type: 'reply', text: '' }], decision: 'not_my_job', confidence: 1 }) };
    const orch = makeOrchestrator({ [FLOW_STATES.CLARIFICATION]: nuc }, { analyze: async () => ({ target: null, confidence: 1 }) });
    await expect(orch.handle({ userId: 'u1', userMessage: 'x' })).resolves.toMatchObject({
      reply: 'Algo deu errado. Pode repetir?',
      origin: 'orchestrator',
    });
  ...
  it('max one routing and second not_my_job forces clarification', async () => {
    const a: Nucleus = { analyze: async () => ({ actions: [], decision: 'not_my_job', confidence: 1 }) };
    const b: Nucleus = { analyze: async () => ({ actions: [], decision: 'not_my_job', confidence: 1 }) };
    const clar: Nucleus = { analyze: async () => ({ actions: [{ type: 'reply', text: 'clar' }], decision: 'handled', confidence: 1 }) };
    const orch = makeOrchestrator({ [FLOW_STATES.GOAL_CREATION]: a, [FLOW_STATES.GOAL_STATUS]: b, [FLOW_STATES.CLARIFICATION]: clar }, { analyze: async () => ({ target: FLOW_STATES.GOAL_STATUS, confidence: 1 }) });
  ...
  it('history always present and state changes only via orchestrator', async () => {
    const nuc: Nucleus = { analyze: async () => ({ actions: [{ type: 'reply', text: 'ok' }], decision: 'handled', confidence: 1 }) };
    const orch = makeOrchestrator({ [FLOW_STATES.CLARIFICATION]: nuc }, { analyze: async () => ({ target: null, confidence: 1 }) });
  ...
  it('concurrency control rejects version conflict', async () => {
    const nuc: Nucleus = { analyze: async () => ({ actions: [], decision: 'handled', confidence: 1 }) };
    const orch = makeOrchestrator({ [FLOW_STATES.CLARIFICATION]: nuc }, { analyze: async () => ({ target: null, confidence: 1 }) });
    // prime session
    const sessService = (orch as any).sessionService as InMemorySessionService;
    ...
    await sessService.updateSessionVersioned('u2', { state: FLOW_STATES.CLARIFICATION }, s0.version);
    // sem ação que escreva sessão, a conversa segue segura e sem crash.
    await expect(orch.handle({ userId: 'u2', userMessage: 'hi' })).resolves.toMatchObject({
      reply: '',
