...
export class CommunicationService {
  constructor(
    @Optional()
    @Inject(forwardRef(() => ConversationAIService))
    private readonly ai: ConversationAIService | null,
  ) {}
  /**
  ...
  async generateSimpleResponse(
    ...
    switch (intent) {
      case 'MARK_DONE':
        return formatMessage(MESSAGES.MARK_DONE_SUCCESS, {
          name,
          comma: name ? ',' : '',
        });
      case 'RESCHEDULE_REMINDER':
        return formatMessage(MESSAGES.RESCHEDULE_SUCCESS, {
          name,
          comma: name ? ',' : '',
        });
      case 'ABANDON_GOAL':
        return formatMessage(MESSAGES.ABANDON_SUCCESS, {
          name,
          comma: name ? ',' : '',
        });
      case 'TIME_RESPONSE':
   ...
   */
  async generateProgressMetadata(
    userId?: string,
    opts?: { goalTitle?: string; userMessage?: string },
  ) {
    ...
    if (userMessage) {
      description =
        userMessage.length > CommunicationService.MAX_DESCRIPTION_LENGTH
          ? userMessage.slice(
              0,
              CommunicationService.MAX_DESCRIPTION_LENGTH - 3,
            ) + '...'
          : userMessage;
    }
    ...
    let title = '';
    try {
      if (!this.ai) throw new Error('ConversationAIService não disponível');
      const prompt =
        `Você gera um título curto e criativo para um evento de progresso de meta. ` +
      ...
      const llm = await this.ai.analyze(
        {
          ...
          payload: { userName, goalTitle, userMessage },
