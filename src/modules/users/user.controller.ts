...
export class UserController {
  ...
  async create(
    @Body()
    body: { name: string; email: string; password: string; timezone?: string },
    @Req() req: any,
  ) {
  ...
  @ApiOperation({
    summary: 'Verificar vinculação Telegram',
    description: 'Verifica se o usuário possui conta Telegram vinculada.',
  })
  @ApiResponse({
    status: 200,
    description: 'Status da vinculação.',
    schema: { type: 'object', properties: { linked: { type: 'boolean' } } },
  })
  ...
  @ApiOperation({
    ...
    description:
      'Verifica se o usuário possui Telegram e/ou WhatsApp vinculados e o canal preferido.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Atualizar mundo atual',
    description: 'Altera o mundo (world) atual do usuário autenticado.',
  })
  @ApiBody({
    schema: {
      type: 'object',
      required: ['currentWorldId'],
      properties: { currentWorldId: { type: 'string' } },
    },
  })
  @ApiResponse({ status: 200, description: 'Mundo atualizado.' })
  async updateCurrentWorld(
    @Body() body: { currentWorldId: string },
    @Req() req: any,
  ) {
