...
export class WorldsPlantedController {
  ...
  @ApiOperation({
    summary: 'Listar árvores plantadas',
    description:
      'Retorna as árvores plantadas do usuário autenticado no mundo especificado.',
  })
  ...
  @ApiQuery({
    name: 'stage',
    required: false,
    description: 'Estágio de crescimento',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
  ...
  async listPlantedTrees(
    ...
    if (!userId)
      throw new BadRequestException(
        'userId não encontrado no contexto da requisição',
      );
    ...
    const plantedTrees = await Promise.all(
      rows.map((row: any) => this.storageService.signPlantedTree(row)),
    );
  ...
  @ApiOperation({
    summary: 'Deletar árvore plantada',
    description:
      'Remove uma árvore plantada específica com seus eventos e metas associadas.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  @ApiParam({ name: 'plantedTreeId', description: 'ID da árvore plantada' })
  @ApiResponse({ status: 200, description: 'Árvore removida.' })
  async deletePlantedTreeById(
    @Param('id') id: string,
    @Param('plantedTreeId') plantedTreeId: string,
    @Req() req: any,
  ) {
    const safeId = String(id || '').replace(/[^a-zA-Z0-9-_]/g, '');
    if (!safeId) throw new BadRequestException('invalid world id');
    ...
    const out = await (prisma as any).$transaction(async (tx: any) => {
      ...
      if (planted.worldId !== safeId)
        throw new BadRequestException(
          'planted tree does not belong to this world',
        );

      const ownedGoal = await tx.goal.findFirst({
        where: { plantedTreeId: pid, userId },
      });
      if (!ownedGoal)
        throw new BadRequestException('planted tree does not belong to user');

      await tx.goal.deleteMany({ where: { plantedTreeId: pid, userId } });

      const deletedEvents = await tx.growthEvent.deleteMany({
        where: { plantedTreeId: pid },
      });
      await tx.plantedTree.delete({ where: { id: pid } });
      return {
        ok: true,
        deletedPlantedTreeId: pid,
        deletedEvents: deletedEvents.count ?? deletedEvents,
      };
    });
  ...
  @ApiOperation({
    summary: 'Limpar todas as árvores plantadas',
    description:
      'Remove todas as árvores plantadas de um mundo. Requer confirm=1.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  @ApiQuery({
    name: 'confirm',
    required: true,
    description: 'Confirmação (1 ou true)',
  })
  @ApiResponse({ status: 200, description: 'Árvores removidas.' })
  async clearPlantedTrees(
    @Param('id') id: string,
    ...
    if (!doClear)
      throw new BadRequestException(
        'confirm=1 is required to clear planted trees',
      );

    const out = await (prisma as any).$transaction(async (tx: any) => {
      const plantedRows = await tx.plantedTree.findMany({
        where: { worldId: safeId },
        select: { id: true },
      });
      const ids = (plantedRows || []).map((r: any) => r.id);
      if (!ids.length)
        return { ok: true, deletedPlantedTrees: 0, deletedEvents: 0 };

      await tx.goal.deleteMany({ where: { plantedTreeId: { in: ids } } });
      await tx.goal.deleteMany({ where: { plantedTreeId: { in: ids } } });

      const deletedEvents = await tx.growthEvent.deleteMany({
        where: { plantedTreeId: { in: ids } },
      });
      const deletedPlanted = await tx.plantedTree.deleteMany({
        where: { id: { in: ids } },
      });
      return {
        ok: true,
        deletedPlantedTrees: deletedPlanted.count ?? deletedPlanted,
        deletedEvents: deletedEvents.count ?? deletedEvents,
      };
    });
