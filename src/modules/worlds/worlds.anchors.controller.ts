...
export class WorldsAnchorsController {
  ...
  @ApiOperation({
    summary: 'Obter config de anchors',
    description:
      'Retorna a configuração de anchors (posições de plantio) de um mundo.',
  })
  ...
  async getAnchorsConfig(@Param('id') id: string, @Res() res: Response) {
    ...
    try {
      ...
      if (saved && saved.anchors) {
        ...
        return res.status(HttpStatus.OK).json({
          ...payload,
          meta: { version: '1.0', source: 'db', updatedAt: saved.updatedAt },
        });
      }
      ...
      return res.status(HttpStatus.NOT_FOUND).json({
        ok: false,
        error:
          'config not found. Use POST /api/worlds/:id/anchors-config/regenerate to generate it.',
      });
    } catch (err) {
      return res
    ...
    return res
      .status(HttpStatus.INTERNAL_SERVER_ERROR)
      .json({ ok: false, error: 'unexpected error' });
  }

  @Get(':id/config')
  @ApiOperation({
    summary: 'Obter config completa do mundo',
    description:
      'Retorna a configuração completa (anchors + metadata) do mundo.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  @ApiResponse({ status: 200, description: 'Config retornada.' })
  @ApiResponse({ status: 404, description: 'Config não encontrada.' })
    ...
    try {
      ...
      if (!row)
        return res
          ...
          .json({ ok: false, error: 'config not found' });
  ...
  @ApiOperation({
    summary: 'Editar nó do config',
    description:
      'Atualiza propriedades de um nó (anchor) específico na configuração do mundo.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        growth: { type: 'number', minimum: 1, maximum: 6 },
      },
    },
  })
  ...
  async patchNode(
    ...
    if (
      growth !== undefined &&
      !(Number.isInteger(growth) && growth >= 1 && growth <= 6)
    ) {
      throw new BadRequestException('growth must be integer between 1 and 6');
    }

    if (
      !((layer && slot) || (typeof x === 'number' && typeof y === 'number'))
    ) {
  ...
  @ApiOperation({
    ...
    description:
      'Reprocessa o SVG e regenera a configuração de anchors do mundo.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  ...
  async regenerateAnchorsConfig(@Param('id') id: string, @Res() res: Response) {
    ...
    try {
      ...
      return res
        ...
        .json({ message: 'Anchors config regenerated' });
    } catch (err) {
      return res
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ ok: false, error: String(err) });
