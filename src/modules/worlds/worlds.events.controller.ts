...
export class WorldsEventsController {
  ...
  @ApiOperation({
    summary: 'Obter progressão de árvore',
    description: 'Retorna a progressão de uma árvore específica em um anchor.',
  })
  ...
  @ApiOperation({
    summary: 'Obter progressão por body',
    description:
      'Retorna a progressão de árvore especificando anchorId no body.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  @ApiResponse({ status: 200, description: 'Progressão retornada.' })
  async getTreeProgressionByBody(
    @Param('id') id: string,
    @Body() body: { anchorId?: string },
  ) {
  ...
  @ApiOperation({
    summary: 'Criar evento de crescimento',
    description:
      'Não cria meta nem planta árvore. O plantio ocorre só em POST /api/goals.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  @ApiResponse({ status: 400, description: 'Use POST /api/goals para plantar.' })
  async createGrowthEventByCatalog(
    ...
    return this.worldsEventsService.createGrowthEventByCatalog(
      id,
      bodyWithUser,
    );
  }
  ...
  @ApiOperation({
    summary: 'Progredir árvore plantada',
    description: 'Avança o estágio de crescimento de uma árvore plantada.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  ...
  async progressPlantedTreeByBody(
    ...
    const userId = req?.user?.userId;
    const bodyWithUser = { ...body, userId };
    return this.worldsEventsService.progressPlantedTree(
      id,
      bodyWithUser as any,
    );
  }
  ...
  @ApiOperation({
    summary: 'Progredir árvore plantada (global)',
    description:
      'Avança o estágio de crescimento de uma árvore plantada sem especificar mundo.',
  })
  @ApiResponse({ status: 201, description: 'Árvore progredida.' })
  async progressPlantedTreeGlobal(
    ...
    const planted = await (prisma as any).plantedTree.findUnique({
