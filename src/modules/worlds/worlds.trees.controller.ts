...
export class WorldsTreesController {
  ...
  @ApiOperation({
    summary: 'Importar árvores do Supabase',
    description:
      'Importa o catálogo de árvores a partir do bucket do Supabase para um mundo específico.',
  })
  ...
  @ApiQuery({
    name: 'family',
    required: false,
    description: 'Família de árvores',
  })
  @ApiQuery({
    name: 'debug',
    required: false,
  ...
  async importFromSupabase(
    ...
    return this.importService.importFromSupabase(
      safeId,
      bucket,
      folder || '',
      family,
  ...
  @ApiOperation({
    summary: 'Importar árvores do Supabase (global)',
    description:
      'Importa árvores sem especificar mundo — usa pasta assets/ por padrão.',
  })
  @ApiQuery({
    name: 'bucket',
    required: true,
  ...
  @ApiQuery({
    name: 'folder',
    required: false,
    description: 'Pasta (padrão: assets)',
  })
  @ApiQuery({
    name: 'family',
    required: false,
  ...
  @Get(':id/trees')
  @ApiOperation({
    summary: 'Listar catálogo de árvores',
    description: 'Retorna todo o catálogo de árvores cadastrado.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  ...
  async listTreeCatalog(@Param('id') id: string) {
    ...
    return Promise.all(
      catalogs.map((catalog) => this.storageService.signTreeCatalog(catalog)),
    );
  }

  @Delete(':id/trees')
  @ApiOperation({
    summary: 'Limpar catálogo de árvores',
    description: 'Remove todas as árvores do catálogo. Requer confirm=1.',
  })
  @ApiParam({ name: 'id', description: 'ID do mundo' })
  @ApiQuery({
    name: 'confirm',
  ...
  async clearTreeCatalog(
    ...
    if (!doClear)
      throw new BadRequestException(
        'confirm=1 is required to clear the catalog',
      );
