...
export class WorldsGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    try {
      const q =
        client.handshake && client.handshake.query
          ...
          : {};
      this.logger.log(
        `socket connected: id=${client.id} addr=${client.handshake.address || 'unknown'} query=${JSON.stringify(q)}`,
      );
      // allow clients to join a world room via query or join event
      const worldId =
        q && (q.worldId || q.room) ? String(q.worldId || q.room) : null;
  ...
  emitTreePlanted(worldId: string, plantedTree: any) {
    try {
      ...
      this.logger.log(
        `emit tree:planted to world:${worldId} plantedId=${plantedTree?.id}`,
      );
    } catch (e) {
  ...
  emitTreeProgress(
    ...
    try {
      const payload = {
        worldId,
        payload: { plantedTreeId, previousStage, stage, actor: actor ?? null },
        emittedAt: new Date().toISOString(),
      };
      this.server?.to(`world:${worldId}`).emit('tree:progress', payload);
      this.logger.log(
        `emit tree:progress to world:${worldId} plantedId=${plantedTreeId} stage=${stage}`,
      );
    } catch (e) {
