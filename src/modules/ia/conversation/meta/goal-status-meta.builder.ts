    ...
    try {
      ...
      const nextReminderFromSchedule = (sc: ScheduleConfig): string | null => {
        ...
        if (sc.type === 'daily' || sc.type === 'weekly') {
          ...
          return userReminderToday > userNow
            ? `hoje às ${timeStr}`
            : `amanhã às ${timeStr}`;
        }
        return null;
      ...
      const nextReminderLabel = (
        ...
        if (
          sc &&
          typeof sc === 'object' &&
          (sc.type === 'once' || sc.type === 'daily' || sc.type === 'weekly')
        ) {
      ...
      const summarizeGoal = (g: any) => {
        const reminderFormatted = g.reminderTime
          ? formatTime(g.reminderTime)
          : null;
        const nextReminder =
          g.scheduleConfig || g.reminderTime
            ...
            : null;
        const scheduleSummary = g.scheduleConfig
          ? formatScheduleSummary(g.scheduleConfig, timezone)
          : null;
        const growthEvents = Array.isArray(g.plantedTree?.growthEvents)
          ? g.plantedTree.growthEvents
          : [];
        ...
        return {
          ...
          growthEvents: growthEvents.map((ge: any) => ({
            createdAt:
              typeof ge.createdAt === 'string'
                ? ge.createdAt
                : new Date(ge.createdAt).toISOString(),
          })),
        };
      };
      ...
      meta.userGoalsSummaryCompletedPontual = (completedPontualGoals || []).map(
        (g: any) => ({
          id: g.id,
          title: g.title,
          type: g.goalType,
          ...
          scheduleConfig: g.scheduleConfig,
          scheduleSummary: g.scheduleConfig
            ? formatScheduleSummary(g.scheduleConfig, timezone)
            : null,
        }),
      );
      ...
      const tomorrowGoals =
        await this.userGoalService.getGoalsForTomorrowForUser(userId, timezone);
      ...
      meta.userGoalsSummaryForToday = todaySummarized.filter((s: any) => {
        ...
        if (
          s?.type === 'Pontual' &&
          (nr.startsWith('amanhã') || nr.startsWith('em '))
        )
          return false;
        return true;
      });
      meta.userGoalsSummaryForTomorrow = tomorrowGoals.map((g: any) =>
        summarizeGoal(g),
      );
    } catch (e) {
