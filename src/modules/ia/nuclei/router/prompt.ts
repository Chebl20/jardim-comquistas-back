...
const SPEC: PromptSpec = {
  ...
  behaviour: {
    ...
    lines: [
      ...
      `"${FLOW_STATES.REMINDER}" é usado quando o usuário RESPONDE a um lembrete já enviado pelo sistema:`,
      `Exemplos que devem ir para "${FLOW_STATES.REMINDER}":`,
      '  • "não me lembre hoje" (resposta a lembrete de meta já existente)',
      '  • "não vou conseguir fazer isso hoje" (respondendo a um lembrete)',
      '  • "pausar por agora" (em contexto de lembrete ativo)',
      '  • "só amanhã" (respondendo a lembrete)',
  ...
  examples: {
    ...
    lines: [
      ...
      `{ "text": "não me lembre hoje", "target": "${FLOW_STATES.REMINDER}", "confidence": 0.9, "reason": "respondendo a lembrete ativo" }`,
      `{ "text": "não vou conseguir fazer isso hoje", "target": "${FLOW_STATES.REMINDER}", "confidence": 0.85, "reason": "skip today em contexto de lembrete" }`,
      `{ "text": "pausar por agora", "target": "${FLOW_STATES.REMINDER}", "confidence": 0.8, "reason": "pausa temporária de lembrete ativo" }`,
